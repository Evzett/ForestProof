...
def _canonical(text: str) -> str:
    """Число к виду, в котором его можно сравнивать: без разделителей и знака.

    Знак снимается намеренно. Сторож ловит выдуманные величины, а не
    выдуманные знаки: в фактах лежит −6485, модель пишет «потеря 6485»,
    и это то же самое число — направление сказано словом. Пока минус
    учитывался, такие ответы отбраковывались целиком, и справка молча
    откатывалась к шаблону ровно на тех участках, где результат
    отрицательный.
    """
    cleaned = text
    for space in ("\u00a0", "\u202f", "\u2009", " "):
