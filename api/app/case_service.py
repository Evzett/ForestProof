...
def _derive_baseline(geometry: dict, aoi_id: str, config) -> tuple[list[dict], float]:
    """Базовая линия по собственной истории пользовательского контура.
...
def calculate(
    ...
    *,
    ...
    derived_baseline = own_geometry or meta is None
    if meta is None:
        ...
        # Покрытие выбирает только растровую вырезку, а не чужую историю.
        # Копия нужна, чтобы не менять кэшированные метаданные каталога.
        meta = {**meta, "baseline_id": DERIVED_BASELINE_ID}
        baseline_rows, _rate = _derive_baseline(geometry, meta["aoi_id"], case.config)
    else:
    ...
    return {
        **area,
        ...
        "baseline_note": (
            ...
            if derived_baseline
            else "Базовая линия участка каталога задана условиями кейса"
        ),
