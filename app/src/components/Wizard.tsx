    ...
    method === "coords" && parsed.points.length >= 3
      ...
      : method === "table" && tableContour
        ...
        : /* Для файла площадь тоже считается по его собственным вершинам.
             Раньше её здесь не было, и на экран шло значение из образца
             разметки — одно и то же «18 200 га» для любого загруженного
             контура. Число на шаге проверки геометрии обязано быть про
             этот контур, иначе проверка ничего не проверяет. */
          method === "file" && geometry
          ? polygonAreaHa(
              (geometry.coordinates as number[][][])[0].map(([lon, lat]) => ({ lat, lon }))
            )
          : null;
